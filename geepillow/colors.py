...
class Color:
    ...
    def from_hex(cls, hex: str):
        ...
        hex_color = hex[0:6]
        lv = len(hex_color)
        convert = lambda i: int(hex_color[i:i + lv // 3], 16)
...
def create(color: Union[str, list, Color]) -> Color:
...
if __name__ == "__main__":
    red = color_from_string("red")
    print(red.rgb())
    print(red.hex())
    print(red.hsv())

    print(Color.from_hex(red.hex()))
    print(Color.from_hsv(*red.hsv()))

    print(Color.from_hex("000000"))
    print(red.hex(opacity=0.5))
